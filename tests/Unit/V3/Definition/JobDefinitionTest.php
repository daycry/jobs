 ...
 * @internal
 */
final class JobDefinitionTest extends CIUnitTestCase
{
    ...
    public function testWithEnabledAndIdempotencyKeyRoundTrip(): void
    {
        $def = (new JobDefinition('command', 'p'))
            ->withEnabled(false)
            ->withIdempotencyKey('abc')
            ->withDependsOn(['x'])
            ->withEnvironments(['staging'])
            ->withCronExpression('*/5 * * * *')
            ->withMeta(['k' => 'v']);

        $this->assertFalse($def->enabled);
        $this->assertSame('abc', $def->idempotencyKey);
        $this->assertSame(['x'], $def->dependsOn);
        $this->assertSame(['staging'], $def->environments);
        $this->assertSame('*/5 * * * *', $def->cronExpression);
        $this->assertSame(['k' => 'v'], $def->meta);
    }
}
